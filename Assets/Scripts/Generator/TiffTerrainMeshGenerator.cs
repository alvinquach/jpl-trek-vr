...
public class TiffTerrainMeshGenerator : IDisposable {

    private Tiff _tiffImage;

    public Tiff TiffImage { get; private set; }

    public TiffTerrainMeshGenerator(Tiff tiffImage) {
        if (tiffImage == null) {
            // TODO Throw exception;
        }
        _tiffImage = tiffImage;
        TiffUtils.PrintInfo(_tiffImage, "TIFF image set:");
    }

    public TiffTerrainMeshGenerator(string filepath) {
        _tiffImage = TiffUtils.FromFilePath(filepath);
        if (_tiffImage == null) {
            // TODO Throw exception;
        }
        TiffUtils.PrintInfo(_tiffImage, $"TIFF loaded from {filepath}");
    }

    // TODO Use a struct or class to pass DEM metadata (ie. scale) to the GenerateMesh methods.
    // TODO Maybe support other image types?

    public Mesh GeneratePlanarMesh(float size, float heightScale, int downsample = 4) {

        // Extract info from TIFF data.
        TiffInfo info = TiffUtils.GetInfo(_tiffImage);
        ...
        byte[] scanline = new byte[_tiffImage.ScanlineSize()];
        ...
        for (int y = 0; y < vVertCount; y++) {
            _tiffImage.ReadScanline(scanline, y * downsample);
    ...
    public Mesh GenerateSphericalMesh(float scale, float heightScale, int downsample = 1) {
